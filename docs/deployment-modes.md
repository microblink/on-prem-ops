...
Microblink Self-Hosted Core uses `DOCVER_WORKFLOW` to select which runtime
...
comprehensive document verification. This mode requires a MicroblinkCore
...
This mode starts the server-side verification models. The active MicroblinkCore
...
-   `DOCVER_WORKFLOW` controls which runtime components and endpoints are
    enabled.
-   The MicroblinkCore license controls which operations the runtime is
